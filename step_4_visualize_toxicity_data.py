...
if __name__ == "__main__":
    ...
    df_amines = pd.read_csv("processed_data/final_db_with_two_level_classificationsUMAP_50_025_0309.csv")
    ...
    # plot(fig, filename="output_html/fig6_amine_atlas_with_toxicity_data_0310.html")
    # df_hit_ratio.to_csv("output_csv/amine_atlas_with_toxicity_data_0310.csv", index=False)
    ...
    fig = px.scatter(df_join_for_plot, x='UMAP-1', y='UMAP-2', color='LogP', template='plotly_white', size='Size',
                     hover_data=["CID", 'RDKIT_SMILES', 'Type', 'Class', 'Subclass'],
                     color_continuous_scale=px.colors.sequential.Bluered, opacity=0.7, width=1000, height=600)
    plot(fig, filename="output_html/fig7_amine_atlas_with_vapor_pressure_size.html")
